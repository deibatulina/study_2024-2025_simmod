...
Модель «Накорми студентов» представляет собой учебный пример **сети Петри**, демонстрирующий базовые принципы дискретного моделирования систем с распределенными ресурсами. Она реализована в среде CPN Tools – специализированном инструменте для работы с раскрашенными сетями Петри (Colored Petri Nets) [@olifer2010;@petri_neural_network].
...
3. Процесс продолжается до исчерпания ресурсов [@gost2018;@cpntools_features;@cpntools_modeling]
...
- Методы **верификации** моделей [@cpntools_features;@cpntools_modeling]
...
- Реализовать модель "Накорми студентов" в CPN Tools;
- Вычислить пространство состояний, сформировать отчет о нем и построить граф [@korolkova_lab_work].
